...
import { createTransfer, getTransfers, getBases, getEquipment } from '../api/api';
...
export default function TransfersPage() {
  ...
  const fetchTransfers = async () => {
    try {
      const res = await getTransfers({ base_id: fromBase });
  ...
  async function submit(e) {
    ...
    try {
      const payload = {
        from_base_id: Number(fromBase),
        to_base_id: Number(toBase),
  ...
  return (
    <div className="p-6 bg-gray-50 min-h-screen">
      <div className="max-w-6xl mx-auto">
        <h1 className="text-4xl font-bold text-gray-900 mb-2">Inter-Base Transfers</h1>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            ...
            <form onSubmit={submit} className="space-y-6">
              ...
              <div>
                ...
                <select
                  ...
                  className="w-full border border-gray-300 p-3 rounded-lg focus:outline-none focus:border-blue-500"
                >
                  {bases.filter(b => String(b.id) !== fromBase).map(b => (
                    <option key={b.id} value={b.id}>{b.name}</option>
                  ))}
                </select>
              </div>
          ...
          <div className="bg-white rounded-lg shadow-lg p-8">
            ...
            {transfers.length > 0 ? (
                ...
                {transfers.map((t) => (
                  <div key={t.id} className="border border-gray-200 rounded-lg p-4 hover:bg-gray-50">
                    <div className="flex justify-between items-start mb-2">
                      <div>
                        <span className="font-medium text-blue-600">{getEquipmentName(t.equipment_id)}</span>
                        <div className="text-xs text-gray-500 mt-1">
                          {getBaseName(t.from_base_id)} → {getBaseName(t.to_base_id)}
                        </div>
                      </div>
              ...
              <p className="text-gray-500 text-center py-8">No transfers for this base</p>
            )}
          </div>
        </div>
      </div>
